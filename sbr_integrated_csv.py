...
#     print("Required torque = {0}, run at phi_dot = {1}".format(tau, phi_dot))
    ...
    theta = (mu[1]) + 1.5708                        # theta output in radians
...
#     mu_theta.append(theta)
    ...
    state_arr[0] = phi * 2 * np.pi       # multiply by 2*pi for phi rev -> phi rads
    state_arr[1] = theta
    state_arr[2] = phi_dot * 2 * np.pi   # multiply by 2*pi for rev/sec -> rad/sec
