...
for i in range(1000):
    ...
    theta_dot = (mu[0] + mu[2])           # radians. 
    theta = mu[1] + (np.pi/2)                       # radians
    mu_dtheta.append(theta_dot)
    mu_theta.append(theta)

dtheta_arr = np.array(mu_dtheta)

theta_arr = np.array(mu_theta)
...
df3 = df[:1000].copy()
plt.figure()
df3['dtheta_estimate'] = dtheta_arr
df3['theta_estimate'] = theta_arr
# df3['arctan2(y/x)'] = y_theta[:1000] / rad_sec
df3['Theta2 (deg)'] = df3['Theta (deg)'] * rad_sec

plt.plot(df3['Gyro45(deg/sec)']*rad_sec, label="Reference w45")
...
plt.plot(df3['dtheta_estimate'], label="Predicted dtheta")
